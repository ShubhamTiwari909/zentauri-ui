...
Roadmap for hardening the existing Payload block schema in `src/blocks/` and
expanding it into a full blog-authoring system. Grounded in the current setup:
Payload 3.85 + MongoDB (mongoose) + UploadThing storage, Lexical editor,
config-level block registry (`blocks: allBlocks` in `payload.config.ts`),
`Pages.layout` = Section blocks, each Section holding rich text with
`BlocksFeature(CONTENT_BLOCK_SLUGS)`, rendered by
`app/(frontend)/components/blocks/{BlockRenderer,converters}.tsx`.
...
7. **Free-form JSON fields are unvalidated** (`json-viewer`, `tree-view`
   presumably accept text parsed with JSON5 at render time) — bad input is
   discovered on the frontend, not in the admin.
...
export const visibilityField = (): Field => ({
  ...
  admin: {
    position: "sidebar-ish (collapsed group)",
    description: "Hide this block on selected viewports",
  },
});
```
...
- Per-item **width** select on each `items` entry… but `items` is a blocks
  ...
  Simpler alternative that avoids a breaking change: keep `items` as-is and
  add a parallel `itemWidths` — **don't**; take the small migration hit now,
  the wrapper array is the correct shape and existing Row usage is young.
...
| ------------------------------- | ------------------------- | ------ | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `chart`                         | `charts/*` (9 types)      | M      | ⭐⭐⭐   | one block, `type: select(area\|bar\|bubble\|funnel\|line\|pie\|radar\|scatter\|stacked-bar)`; data as `json` field with `json5Validator` + per-type shape validation; title, height, legend/tooltip toggles. Charts are client components — converter wraps in a lazy client boundary so a chartless post ships no Recharts. |
...
| `data-table`                    | `data-table`              | M      | ⭐⭐     | columns array + `json` rows (validated against columns in a block-level `validate`), sortable/paginated toggles; supersedes static `table` for data-heavy posts                                                                                                                                                              |
