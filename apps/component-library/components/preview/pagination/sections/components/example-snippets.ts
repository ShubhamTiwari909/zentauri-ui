...
export function paginationControlledExampleSnippet(): string {
  return `${variantLeadComment("mode · controlled + compact")}import { useState } from "react";
import {
  Pagination,
  ...
  return (
    <div>
      ...
      onPageChange={setPage}
        ...
        boundaryCount={1}
      />
      <p className="text-xs text-slate-800 dark:text-slate-400 mt-5">Current page: <span className="font-bold">{page}</span></p>
    </div>
  );
}`;
}

export function paginationHeadlessExampleSnippet(): string {
  return `${variantLeadComment("mode · headless usePagination")}import { useState } from "react";
import {
  usePagination,
} from "@zentauri-ui/zentauri-components/ui/pagination";
