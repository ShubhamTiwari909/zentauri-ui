...
const SKELETON_APPEARANCES = ["default", "subtle", "muted"] as const satisfies readonly NonNullable<
  SkeletonProps["appearance"]
>[];

const SKELETON_SIZES = ["sm", "md", "lg"] as const satisfies readonly NonNullable<SkeletonProps["size"]>[];

const SKELETON_ROUNDED = ["none", "sm", "md", "lg", "full"] as const satisfies readonly NonNullable<
...
export function SkeletonCodeExamplesSection() {
  return (
    <section className={SECTION}>
      <h2 className="mt-3 text-2xl font-semibold text-white">Skeleton variants examples</h2>
      ...
      <div className="mt-6 space-y-10 rounded-xl">
        ...
        {SKELETON_SIZES.map((size) => (
          <PreviewCodeShowcase
            key={`size-${size}`}
            code={skeletonSnippet({ appearance: "default", size, rounded: "md" })}
          >
        ...
        {SKELETON_ROUNDED.map((rounded) => (
          <PreviewCodeShowcase
            key={`rounded-${rounded}`}
            code={skeletonSnippet({ appearance: "subtle", size: "md", rounded })}
          >
