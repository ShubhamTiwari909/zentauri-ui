...
const TABS_APPEARANCES = ["default", "pill", "underline"] as const satisfies readonly NonNullable<
  TabsProps["appearance"]
>[];

const TABS_SIZES = ["sm", "md", "lg"] as const satisfies readonly NonNullable<TabsProps["size"]>[];
...
function TabsDemo(opts: {
  ...
  const { appearance, size } = opts;
  return (
    <Tabs defaultValue="one" appearance={appearance} size={size} animation="fade">
      <TabsList>
        <TabsTrigger value="one">One</TabsTrigger>
        <TabsTrigger value="two">Two</TabsTrigger>
      </TabsList>
  ...
  return (
    <section className={SECTION}>
      ...
      <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
        List chrome and density; list and triggers share the same appearance context. Each snippet starts with Variant: for the row.
      </p>
