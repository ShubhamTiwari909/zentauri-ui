  ...
  return (
    <section className={INPUTS_CODE_EXAMPLES_SECTION_CLASS}>
      ...
      <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
        Use Show output / Show code on each row to compare the live control with the JSX.
        Each snippet opens with a Variant line naming the axis and token.
      </p>
      <div className="mt-6 space-y-10 rounded-xl">
        {appearanceInputsExtended.map((row) => (
          <PreviewCodeShowcase key={row.label} code={inputAppearanceSnippet(row)}>
        ...
        {animationPresets.map((preset) => {
          const [label] = preset;
          return (
            <PreviewCodeShowcase key={`anim-${label}`} code={inputAnimationSnippet(preset)}>
              <p className="mb-5 text-xs font-semibold text-white">
                Animation: <span className="font-bold">{preset[1].toUpperCase()}</span>
              </p>
        ...
        {sizeInputs.map((row) => (
          <PreviewCodeShowcase key={`size-${row.label}`} code={inputSizeSnippet(row)}>
            <p className="mb-5 text-xs font-semibold text-white">
        ...
        <PreviewCodeShowcase key="textarea-showcase" code={inputTextareaSnippet()}>
          <p className="mb-5 text-xs font-semibold text-white">
        ...
        <PreviewCodeShowcase key="disabled-showcase" code={inputDisabledSnippet()}>
          <p className="mb-5 text-xs font-semibold text-white">
            State: <span className="font-bold">DISABLED</span>
          </p>
          <InputsDisabledDemo />
        </PreviewCodeShowcase>
        <PreviewCodeShowcase key="readonly-showcase" code={inputReadOnlySnippet()}>
          <p className="mb-5 text-xs font-semibold text-white">
            State: <span className="font-bold">READ ONLY</span>
          </p>
          <InputsReadOnlyDemo />
        </PreviewCodeShowcase>
        <PreviewCodeShowcase key="controlled-showcase" code={inputControlledSnippet()}>
          <p className="mb-5 text-xs font-semibold text-white">
            Pattern: <span className="font-bold">CONTROLLED</span>
          </p>
          <InputsControlledDemo />
        </PreviewCodeShowcase>
        <PreviewCodeShowcase key="error-message-showcase" code={inputErrorMessageSnippet()}>
          <p className="mb-5 text-xs font-semibold text-white">
        ...
        <PreviewCodeShowcase key="checkbox-showcase" code={inputCheckboxSnippet()}>
          <p className="mb-5 text-xs font-semibold text-white">
        ...
        <PreviewCodeShowcase key="radio-group-showcase" code={inputRadioGroupSnippet()}>
          <p className="mb-5 text-xs font-semibold text-white">
